...
export const metadata: Metadata = {
  ...
  title: {
    ...
    default: "TALOS（タロス）ワンウェイケグ｜使い捨てビールケグでコスト削減 - セレクトモール",
  },
  
  // 説明文を155文字以内に最適化し、具体的数値を追加
  description: "TALOS（タロス）ワンウェイケグは使い捨てタイプのビールケグです。ステンレスケグと比較して運用コスト60%削減、軽量設計（1.3kg）で輸送費削減を実現。S/A/G/Tタイプ対応で既存設備利用可能。海外展開・小規模ブルワリー向け。",

  // キーワード追加（Next.js 13+では非推奨だが、一部検索エンジンで有効）
  keywords: [
    "ワンウェイケグ", "TALOS", "タロス", "ビールケグ", "使い捨てケグ",
    "ステンレスケグ", "Petainer", "ペタイナー", "Keykeg", "キーケグ",
    "ブルワリー", "コスト削減", "海外輸出", "軽量ケグ", "T-KEG"
  ],
  ...
  openGraph: {
    ...
    // OG説明文を200文字以内で最適化
    description: "TALOS（タロス）ワンウェイケグなら運用コスト60%削減！軽量1.3kgで輸送費も削減。ステンレスケグの回収・洗浄・メンテナンス不要。S/A/G/Tタイプ対応で既存設備そのまま利用可能。海外展開・小規模ブルワリーに最適なソリューションです。",
  ...
  twitter: {
    ...
    description: "軽量1.3kgの使い捨てビールケグでコスト革命！ステンレスケグと比較して運用コスト60%削減。回収・洗浄・メンテナンス不要。S/A/G/Tタイプ対応で既存設備利用可能。海外展開・小規模ブルワリーの課題を解決します。",
  ...
  other: {
    "application/ld+json": JSON.stringify({
      ...
      "@type": "Product",
      "name": "TALOS ワンウェイケグ",
      "description": "使い捨てタイプのビールケグ。運用コスト60%削減、軽量1.3kg設計",
      "brand": {
        "@type": "Brand",
        "name": "TALOS"
      },
